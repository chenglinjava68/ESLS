...
public class BalanceController {
    ...
    public ResponseEntity<ResultBean> getLogs(@RequestParam(required = false) String query, @RequestParam(required = false) String queryString, @Min(message = "data.page.min", value = 0)@RequestParam(required = false) Integer page, @Min(message = "data.count.min", value = 0) @RequestParam(required = false) Integer count) {
        ...
        if(query!=null && query.contains(" ")){
            List content = balanceService.findAllBySql(TableConstant.TABLE_LOGS, "like", query, queryString, page, count, Balance.class);
        ...
        if(result.equals(ConditionUtil.QUERY_ATTRIBUTE_PAGE)) {
            List list = balanceService.findAll();
            List content = balanceService.findAllBySql(TableConstant.TABLE_LOGS, query, queryString, page, count,Balance.class);
            return new ResponseEntity<>(new ResultBean(content, list.size()), HttpStatus.OK);
        }
    ...
    //sendGetBalance sendBalanceToZero sendBalanceToFlay sendGetBalancePower
    ...
    public ResponseEntity<ResultBean> sendGetBalance(@RequestBody @ApiParam("标签信息集合") RequestBean requestBean, @RequestParam Integer mode) {
        ...
        if(mode==0){
            responseBean = balanceService.sendGetBalance(requestBean);
        }
        else if(mode==1){
            responseBean = balanceService.sendBalanceToZero(requestBean);
        }
        else if(mode==2){
            responseBean = balanceService.sendBalanceToFlay(requestBean);
        }
        else if(mode==3){
            responseBean = balanceService.sendGetBalancePower(requestBean);
        }
